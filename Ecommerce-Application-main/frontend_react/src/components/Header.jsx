...
import logo from '../assests/homepage_logo.png';
import { useNavigate } from 'react-router-dom';
import CartContext from "../context/CartContext";
import { RiAdminFill } from 'react-icons/ri';

const Header = ({ user, setSearchTerm }) => {
    ...
    <section className='bg-gray-900'>
      <div className='max-w-7xl flex mx-auto p-2 items-center justify-between gap-10'>
        <a href='/' className='flex flex-col items-start justify-start'>
          <img src={logo} alt="company-logo" className='h-[60px] pointer-events-none'/>
        </a>
        ...
        <div className='hidden lg:flex items-center space-x-6 text-white'>
          <a href={user ? '/account' : '/login'} className="flex flex-col items-center">
        ...
        {user &&
          <div className='hidden lg:flex items-center space-x-6 text-white'>
            <a href={`/${user?._id?.$oid}/orders`} className="flex items-center font-bold">
              Orders
            </a>
          </div>
        }
